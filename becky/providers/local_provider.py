...
class LocalProvider:
    ...
    def backup_files(self, new_files):
        """
        ...
        for file_in_index, file_in in enumerate(new_files):
            if os.path.isdir(file_in): 
                saved_files.append({'name': file_in, 'type': 'directory', 'date': current_time})
                continue
            else:
                file_out = self._generate_output_path(copy_path)
                self._copy_file(file_in, file_out)
                saved_files.append({'name': file_in, 'path': file_out, 'type': 'file', 'date': current_time})
        return saved_files
